...
public class SCLScanner {
	...
	/**
	 * Private instances of the class SCLScanner
	 * tokens
	 */
	private List<Token> tokens;
	private List<String> keywords;
	private String filename;
	private HashMap<String, TokenType> hashMap= new HashMap<String, TokenType>();
	
	public SCLScanner(String file_name) throws FileNotFoundException, LexicalException { 
		this.filename = file_name;
		assert(filename != null);
		tokens = new ArrayList<Token>();
		// List of keywords
		keywords = new ArrayList<String>(Arrays.asList("symbol", "forward","references", "function", "pointer", "array",
										"type", "struct", "integer", "enum", "global", "declarations", "implementations", "main",
										"parameters","constant", "begin", "endfun", "enfor", "if", "else", "endif", "repeat", "until",
										"endrepeat", "display", "set",  "return"));
		// Getting the file content into  a buffer reader
		BufferedReader sourceProgram = new BufferedReader(new FileReader(System.getProperty("user.dir")+"/"+filename));
		int lineNum = 0; 
		String getLine;
		try{
	...
	public void lexicalAnalyzerOutput() {
		try {
			String output = "", filenam = "output"+"_"+this.filename+".txt";
			for(int i = 0; i < tokens.size(); i++) 
				output += tokens.get(i).toString()+"\n";
			File file = new File(filenam);
			if(!file.exists())
				file.createNewFile();
			PrintWriter writer = new PrintWriter(filenam, "UTF-8");
